...
test.describe("Admin Kanban - Basic Flow", { tag: ["@admin"] }, () => {
  ...
  test(
    "Requests tab and Inquiries board are visible",
    { tag: ["@smoke"] },
    async ({ adminPage }) => {
      const admin = new AdminPage(adminPage);
  ...
  test("expand first lane and verify request columns", async ({
    adminPage,
  }) => {
...
test.describe("Admin Kanban - Request Edit Flow", { tag: ["@admin"] }, () => {
  ...
  test("open timeline drawer and edit a request", async ({
    adminPage,
    testRunId,
  }) => {
    const admin = new AdminPage(adminPage);
    ...
    const targetItem = listItems
      .filter({ hasText: "E2E test question" })
      .first();
...
test.describe("Admin Kanban - Request Delete Flow", { tag: ["@admin"] }, () => {
  ...
  test(
    "delete a request from timeline drawer",
    { tag: ["@destructive"] },
    async ({ adminPage }) => {
      const admin = new AdminPage(adminPage);
      const adminEmail = process.env.E2E_ADMIN_EMAIL;
