...
test.describe(
  "Admin Analytics - Topic Recompute Flow",
  { tag: ["@admin"] },
  () => {
    test.beforeEach(() => {
    ...
    test(
      "recompute topics shows success alert and refreshes table",
      { tag: ["@slow"] },
      async ({ adminPage }) => {
        const admin = new AdminPage(adminPage);
        await admin.navigateToAnalytics();
    ...
    test(
      "recompute topics button disables during analysis",
      { tag: ["@slow"] },
      async ({ adminPage }) => {
        const admin = new AdminPage(adminPage);
        await admin.navigateToAnalytics();
...
test.describe(
  "Admin Analytics - Super-Admin Management",
  { tag: ["@admin"] },
  () => {
    ...
    test("Settings tab is visible for super-admin", async ({
      superAdminPage,
    }) => {
      const admin = new AdminPage(superAdminPage);
    ...
    test("Settings tab shows Manage Admins form", async ({
      superAdminPage,
    }) => {
      const admin = new AdminPage(superAdminPage);
    ...
    test(
      "add admin shows success alert",
      { tag: ["@destructive"] },
      async ({ superAdminPage }) => {
        test.skip(
    ...
    test(
      "remove admin shows success alert",
      { tag: ["@destructive"] },
      async ({ superAdminPage }) => {
        test.skip(
          !isConfigured(MANAGE_ADMIN_TARGET),
    ...
    test("manage admin shows error alert on failure", async ({
      superAdminPage,
    }) => {
      const admin = new AdminPage(superAdminPage);
      await admin.navigateToSettings();
...
test.describe(
  "Admin Analytics - Settings Tab Visibility",
  { tag: ["@admin"] },
  () => {
    test.beforeEach(() => {
      test.skip(!isConfigured(ADMIN), "Admin E2E credentials not configured");
    ...
    test("Settings tab is NOT visible for regular admin", async ({
      adminPage,
    }) => {
      await adminPage.goto("/admin");
