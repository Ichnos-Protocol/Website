...
test.describe("Contact Page - Submit Inquiry", { tag: ["@contact"] }, () => {
  ...
  test("submit an inquiry successfully", async ({ page }) => {
    ...
    await expect(
      contact.contactModal.getByText("Submit an Inquiry"),
    ).toBeVisible();
    ...
    await expect(
      contact.contactModal.getByRole("button", { name: "Book a Meeting" }),
    ).toBeVisible();
  });
});

test.describe(
  "Contact Page - Auth-interrupted Submission",
  { tag: ["@contact"] },
  () => {
    test.beforeEach(async ({ page }) => {
      await waitForAppReady(page, "/contact");
    ...
    test("shows auth modal for unauthenticated user", async ({ page }) => {
      ...
      await expect(auth.authModal.getByLabel("Email")).toBeVisible();
...
test.describe(
  "Contact Page - Returning User Flow",
  { tag: ["@contact"] },
  () => {
    test.beforeEach(async ({ page }) => {
      test.skip(!isConfigured(USER), "User E2E credentials not configured");
    ...
    test("shows inquiry status list for returning user", async ({ page }) => {
      ...
      const inquiryRow = page
        .locator(".list-group-item")
        .filter({ hasText: "Existing inquiry question" });
