...
vi.mock("../organisms/DeleteAccountModal", () => ({
  default: function MockDeleteAccountModal({
    show,
    onClose,
    onConfirm,
    isLoading,
...
describe("PrivacyPage", () => {
  ...
  it("renders Privacy Policy section", () => {
    ...
    expect(
      screen.getByText(/do not sell or share your data/),
    ).toBeInTheDocument();
  });
  ...
  it("renders the consortium campaign-source disclosure", () => {
    ...
    expect(
      screen.getByText(
        /sent to us solely if you submit a consortium registration/,
      ),
    ).toBeInTheDocument();
  });
  ...
  it("shows download success alert after download", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Your data has been downloaded."),
      ).toBeInTheDocument();
    });
  });
