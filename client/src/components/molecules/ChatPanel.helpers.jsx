...
export async function refreshHistorySafely(
  triggerHistory,
  fetchGenRef,
  dispatch,
) {
  ...
  try {
    ...
    if (fetchGenRef.current === gen)
      dispatch(setMessages(mapHistoryToMessages(r.data)));
  } catch {
    /* keep optimistic */
  }
}
...
// optimistic messages.
export function fetchHistoryGuarded(
  triggerHistory,
  fetchGenRef,
  sendPendingRef,
  dispatch,
...
export function buildLocalStreamOptions(local) {
  return {
    ...
    onAiMessage: (m) => local.setMessages((p) => [...p, m]),
    onLoadingChange: local.setLoading,
    onError: local.setError,
    onDailyCount: local.setDailyCount,
  };
}
