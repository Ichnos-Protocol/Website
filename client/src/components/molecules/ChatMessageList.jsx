...
const bubbleClass = (role) =>
  `chat-panel__bubble chat-panel__bubble--${role === "user" ? "user" : "bot"}`;
const bubbleTestId = (role) =>
  ...
  return (
    ...
    >
      <div
        ...
        className="chat-panel__bubble chat-panel__bubble--bot"
      >
        {WELCOME_TEXT}
      </div>
      {messages.map((m, i) => (
        <div
          key={i}
          data-testid={bubbleTestId(m.role)}
      ...
      {error === "ai_unavailable" && (
        <Alert variant="warning" className="mt-2">
          {AI_UNAVAILABLE_ALERT}
          <div className="mt-2">
            <Button variant="primary" size="sm" onClick={onContactRedirect}>
              Leave your question
            </Button>
          </div>
        </Alert>
