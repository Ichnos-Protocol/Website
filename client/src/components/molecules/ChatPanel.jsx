...
export default function ChatPanel({ mode, persistState }) {
  ...
  } = useChatPanel({ mode, persistState });

  return (
    <div
      data-testid="chat-panel"
      className={`chat-panel ${isModal ? "chat-panel--modal" : "chat-panel--inline"}`}
    >
      <ChatMessageList
        listRef={listRef}
        messages={messages}
      ...
      />
      <Badge bg="secondary" className="chat-panel__count">
      ...
      <p data-testid="chat-panel-disclaimer" className="chat-panel__disclaimer">
        {DISCLAIMER_TEXT}
      </p>
    </div>
  );
