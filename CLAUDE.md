...
- **Max file length**: 200 lines for **application JavaScript modules** (raised from 120 by owner ruling on 2026-09-22, so that cohesive content-constant files such as `readinessAssessmentContent.js` are not split against their grain). The cap covers components, hooks, services, helpers, repositories, middleware, routes and controllers under `client/src`, `server/src` and `server/api`, plus any file that `client/src`, `server/src` or `server/api` imports, wherever it sits. If such a module exceeds this, refactor into smaller modules.
  - **Test files are exempt.** A `.test.js`/`.test.jsx` file grows with the number of cases it covers, and splitting one to hit a line count scatters related assertions across files for no gain. After formatting, 52 test files are over 200 lines (50 `.test.js`/`.test.jsx`, 2 e2e `.spec.js`; remeasured 2026-09-24 after VF-P13a deleted the Storage repository test), the largest `client/src/components/organisms/ChatModal.test.jsx` at 821 (782 unformatted). That is acceptable; splitting a test file is a decision about cohesion, never about length.
  - **Executable application modules stay capped at 200.** Components, hooks, services, helpers, repositories, middleware, routes and controllers are held to the cap whatever directory they sit in.
  - **Content files are exempt in the same way test files are.** A JavaScript file whose length follows the copy or declarative data it holds (strings, arrays or objects of records, schema literals), and not branching logic, is exempt. The current content-exempt files, with line counts measured after P13a-C1/C2 on 2026-09-24 (`wc -l`, HEAD → formatted):
    - `client/src/constants/readinessAssessmentContent.js` (239 → 315): page copy for the readiness assessment; the five pricing and interpolation helpers at the top total under 50 lines.
    - `client/src/constants/vocabulary.js` (224 → 224): pattern and file lists for the vocabulary guard; the only logic is a 10-line directory walk.
    - `client/src/constants/services.js` (205 → 205): service records and pillar definitions; the only logic is a one-line filter.
  - **Stylesheets are outside the cap.** The cap applies to JavaScript modules, and a stylesheet is not one. `client/src/index.css` measures 1414 → 1426. It is not a content exemption and not grandfathered; it is simply not in scope.
  - **Content-exempt is not grandfathered.** A new cohesive content module needs no waiver. A mixed logic file never qualifies, whatever its path and whatever its size: living under `constants/` or being long does not make it content. The test is what drives the length.
  - **Modules over 200 after formatting.** Measured 2026-09-24 in a disposable worktree after P13a-C1/C2, running each package's own `format` script (Prettier 3.8.1), then `wc -l` on every tracked file, HEAD → formatted.
    - No application JavaScript module outside the content-exempt and grandfathered lists is over 200 after formatting. The C1/C2 modules measure `client/src/components/molecules/ChatPanel.jsx` (27 → 72), `client/src/components/molecules/ChatMessageList.jsx` (33 → 84), `client/src/components/molecules/ChatPanel.helpers.jsx` (35 → 59), `client/src/hooks/useChatPanel.js` (88 → 94), `client/src/hooks/useChatPanelActions.js` (66 → 70), `client/src/hooks/useChatPanelEffects.js` (60 → 60), `server/src/repositories/knowledgeRepository.js` (93 → 96, remeasured after VF-P13a).
    - Grandfathered (next bullet): `server/src/repositories/adminRepository.js` (353 → 362), `server/src/repositories/userRepository.js` (255 → 269), `server/src/controllers/adminController.js` (252 → 252), `server/src/services/adminService.js` (223 → 223), `server/scripts/seedE2EOnPreview.js` (305 → 332).
    - Outside cap scope, not grandfathered: the standalone CLI scripts `server/scripts/verifyFirestoreIngestion.js` (1026 → 1026), `server/scripts/utils/deduplicateFirestore.js` (496 → 496), `server/scripts/extractMarkdownKnowledge.js` (277 → 277), `server/scripts/extractWebKnowledge.js` (202 → 202). Nothing under `server/src` or `server/api` imports them.
    - Test-exempt: the e2e fixture `e2e/tests/fixtures/auth.js` (213 → 218) and the specs `e2e/tests/admin/admin-analytics.spec.js` (330 → 370) and `e2e/tests/admin/admin-kanban.spec.js` (233 → 252). They are test code, not application exceptions.
  - Five application files currently exceed the cap and are grandfathered: `server/src/repositories/adminRepository.js`, `server/src/repositories/userRepository.js`, `server/src/controllers/adminController.js`, `server/src/services/adminService.js` and `server/scripts/seedE2EOnPreview.js` (counts in the bullet above). `seedE2EOnPreview.js` sits outside `server/src` but is application code, because `server/src/app.js` imports it. `client/src/constants/structuredData.js` left the list when P8 executed the readiness assessment spec §7.2.1 split into `serviceSchemas.js`. Do not refactor them as drive-by work; split one only when you are already changing it for another reason.
...
**Prettier: configured, not yet run, do not run it as drive-by work.** As of P13a (2026-09-24) the repository has a root `.prettierrc.json` holding only `endOfLine: "auto"`, a `.prettierignore` in each of `client/`, `server/` and `e2e/`, and narrowed `format` / `format:check` scripts in all three packages: `client` covers `src/**/*.{js,jsx,css}`, `server` covers `src/**/*.js`, `scripts/**/*.js` and `api/**/*.js`, `e2e` covers `**/*.js`. Markdown, JSON and SQL are out by construction. `e2e` gained `prettier` 3.8.1 as a devDependency so its scripts run. The corpus has still never been formatted.

**Measured dry-run baseline, 2026-09-24** (remeasured after VF-P13a removed the Storage repository and its test; Prettier 3.8.1 run with the scripts' own globs in a disposable worktree)**:** `client` 202 of 275 files change, `server` 59 of 125, `e2e` 35 of 45. Each package completes in about two seconds, npm startup included, and no ignored path is touched. Quote style is split roughly 157 single / 89 double across client JavaScript files, with no file internally mixed.

Consequences, so nobody rediscovers this the hard way:

- `format:check` fails in all three packages until P13b formats the corpus. It is deliberately **not** a gate: it is not in the checklist above and no CI job calls it.
- **Do not run `npm run format` to tidy up.** It would rewrite the 296 files above. The historical P13a baseline was 298 (client 202 / server 61 / e2e 35); T16's known two-file delta is reckoned against that figure. A several-hundred-file reformat landing mid-epic makes every review diff unreadable and collides with every in-flight branch.
- `server/.prettierignore` excludes `knowledge-base/`, 270 MB in 366 ignored files (PDFs, SQLite databases with journals, a Playwright browser profile). Prettier reads only the `.gitignore` in its working directory, and `server/.gitignore` lists only `.vercel`; the root `.gitignore` that ignores the knowledge base is never consulted, so the ignore file must stay.
- The single formatting run is P13b in `docs/september_fixes_spec.md`: run `npm run format` once across all three packages in a commit that changes nothing else, then add `format:check` to this checklist. Never run it inside another change.
