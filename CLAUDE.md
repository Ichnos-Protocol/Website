...
- **Max file length**: 200 lines for **application JavaScript modules** (raised from 120 by owner ruling on 2026-09-22, so that cohesive content-constant files such as `readinessAssessmentContent.js` are not split against their grain). The cap covers components, hooks, services, helpers, repositories, middleware, routes and controllers under `client/src`, `server/src` and `server/api`, plus any file that `client/src`, `server/src` or `server/api` imports, wherever it sits. If such a module exceeds this, refactor into smaller modules.
  - **Test files are exempt.** A `.test.js`/`.test.jsx` file grows with the number of cases it covers, and splitting one to hit a line count scatters related assertions across files for no gain. 52 test files are over 200 lines (50 `.test.js`/`.test.jsx`, 2 e2e `.spec.js`; `wc -l` on the formatted tree, 2026-09-24, after the P13b formatting commit), the largest `client/src/components/organisms/ChatModal.test.jsx` at 821. That is acceptable; splitting a test file is a decision about cohesion, never about length.
  ...
  - **Content files are exempt in the same way test files are.** A JavaScript file whose length follows the copy or declarative data it holds (strings, arrays or objects of records, schema literals), and not branching logic, is exempt. The current content-exempt files, with line counts measured by `wc -l` on the formatted tree on 2026-09-24, after the P13b formatting commit:
    - `client/src/constants/readinessAssessmentContent.js` (315): page copy for the readiness assessment; the five pricing and interpolation helpers at the top total under 50 lines.
    - `client/src/constants/vocabulary.js` (224): pattern and file lists for the vocabulary guard; the only logic is a 10-line directory walk.
    - `client/src/constants/services.js` (205): service records and pillar definitions; the only logic is a one-line filter.
  - **Stylesheets are outside the cap.** The cap applies to JavaScript modules, and a stylesheet is not one. `client/src/index.css` measures 1426. It is not a content exemption and not grandfathered; it is simply not in scope.
  ...
  - **Modules over 200.** Measured 2026-09-24 by `wc -l` on every tracked file of the formatted tree, after the P13b formatting commit.
    - No application JavaScript module outside the content-exempt and grandfathered lists is over 200. The C1/C2 modules measure `client/src/components/molecules/ChatPanel.jsx` (72), `client/src/components/molecules/ChatMessageList.jsx` (84), `client/src/components/molecules/ChatPanel.helpers.jsx` (59), `client/src/hooks/useChatPanel.js` (94), `client/src/hooks/useChatPanelActions.js` (70), `client/src/hooks/useChatPanelEffects.js` (60), `server/src/repositories/knowledgeRepository.js` (96).
    - Grandfathered (next bullet): `server/src/repositories/adminRepository.js` (362), `server/src/repositories/userRepository.js` (269), `server/src/controllers/adminController.js` (252), `server/src/services/adminService.js` (223), `server/scripts/seedE2EOnPreview.js` (332).
    - Outside cap scope, not grandfathered: the standalone CLI scripts `server/scripts/verifyFirestoreIngestion.js` (1026), `server/scripts/utils/deduplicateFirestore.js` (496), `server/scripts/extractMarkdownKnowledge.js` (277), `server/scripts/extractWebKnowledge.js` (202). Nothing under `server/src` or `server/api` imports them.
    - Test-exempt: the e2e fixture `e2e/tests/fixtures/auth.js` (218) and the specs `e2e/tests/admin/admin-analytics.spec.js` (370) and `e2e/tests/admin/admin-kanban.spec.js` (252). They are test code, not application exceptions.
...
**Known-good baseline, measured 2026-09-24** (after the P13b formatting commit)**:** `client` 106 test files / 1023 tests green; `server` 48 files / 683 tests green, with 4 files and 20 tests skipped (52 / 703 total); `e2e` `test:unit` 8 files / 108 tests green; both lints clean; `format:check` green in `client`, `server` and `e2e`; client build green. If your run differs from this, you changed something. Do not start a phase from a red tree.
