...
- **Max file length**: 200 lines for **application JavaScript modules** (raised from 120 by owner ruling on 2026-09-22, so that cohesive content-constant files such as `readinessAssessmentContent.js` are not split against their grain). The cap covers components, hooks, services, helpers, repositories, middleware, routes and controllers under `client/src`, `server/src` and `server/api`, plus any file that `client/src`, `server/src` or `server/api` imports, wherever it sits. If such a module exceeds this, refactor into smaller modules.
  - **Test files are exempt.** A `.test.js`/`.test.jsx` file grows with the number of cases it covers, and splitting one to hit a line count scatters related assertions across files for no gain. After formatting, 53 test files are over 200 lines (51 `.test.js`/`.test.jsx`, 2 e2e `.spec.js`), the largest `client/src/components/organisms/ChatModal.test.jsx` at 821 (782 unformatted). That is acceptable; splitting a test file is a decision about cohesion, never about length.
...
**Measured dry-run baseline, 2026-09-24** (P13a, Prettier 3.8.1 run with the scripts' own globs in a disposable worktree after P13a-C1/C2)**:** `client` 202 of 275 files change, `server` 61 of 127, `e2e` 35 of 45. Each package completes in about two seconds, npm startup included, and no ignored path is touched. Quote style is split roughly 157 single / 89 double across client JavaScript files, with no file internally mixed.
...
- **Do not run `npm run format` to tidy up.** It would rewrite the 298 files above. A several-hundred-file reformat landing mid-epic makes every review diff unreadable and collides with every in-flight branch.
