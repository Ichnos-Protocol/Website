...
**Version 1.9, 2026-09-22 · Status: normative · in execution, paused at T8**
...
7. **Dates, amended v1.9.** Two assertions replace v1.8's requirement that both Panel B and the §4.2.2 window render an interpolated date; the window renders no date by design (§4.2.2) and the old item asserted something the fenced copy never contained. (a) Panel B's date equals the interpolated `regulatoryDates.js` value. (b) **No date literal appears anywhere in the page subtree**, asserted as a not-contains over the rendered output for a year pattern and for written month-year forms. (b) is the stronger guard and the one §3 rule 4 actually asks for: it covers the window, the FAQ, the deliverables and every section added later, rather than naming two places and leaving the rest unguarded.
