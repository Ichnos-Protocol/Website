# Ichnos Protocol — Website Spec v5.0

**Version:** 5.0 — late June 2026 (replaces v4.0 as the authoritative spec)
**Owner:** Francesco
**Target site:** `ichnos-protocol.com`
**Strategic basis:** Aligned with `Ichnos_CatenaX_Strategy_Memo_v2.docx` (28 June 2026).
...
>
> **Relationship to prior specs.** v4 (`ichnos_website_CatenaX_pivot_spec_v4.md`)
> is superseded but kept in the repo for context. v3 and v2-era code state
> (commit `ed3f326~1`) are referenced where the diff matters.

## 0e. What's new in v5 (delta from v4)

v5 is a refinement of v4, not a rewrite. Most of v4 carries forward
unchanged. The deliberate changes:

1. **Naming convention rule (§9.6, new).** Code identifiers use
   `CatenaX` (PascalCase, no hyphen, single token). User-facing copy
   continues to use the official spelling `Catena-X` (hyphenated). The
   v4 branch had `CatenaThemeLayout.jsx` (missing the X); v5 fixes
   that to `CatenaXThemeLayout.jsx` and applies the rule across all
   code identifiers.

2. **§4.2 "Battery Passport Integration" service refined.** Tagline
   and description rewritten to align with the strategy memo's
   data-provisioning framing (schema mapping, supplier ingestion
   workflows, CX-0143 / CX-0026 references) while staying in the
   advisory register on the public site. Title kept short for the
   card grid.

3. **Three new sub-sections added to `/passport`:**
   - **§5.10 Build stack credibility** — names the Tractus-X / EDC /
     CX-0143 / CX-0026 / Cofinity-X technical stack as ambient
     terminology so a Catena-X reviewer reads competence. Not a
     marketing pitch.
   - **§5.11 Customer segmentation** — names the three target customer
     groups (ASEAN battery SMEs, EU importers / OEMs, passport-app
     vendors as partners) without disclosing the SaaS pricing model.
   - **§5.12 Roadmap signal** — a single line of measured ambition
...
The disciplined position: the website is a credibility surface for
the **Catena-X consultant qualification listing** and a door-opener for
the advisory conversations that seed the SaaS. The product itself
stays out of public view until business signals justify surfacing it.
...
5. Build stack credibility (§5.10) — **new in v5**
6. Ichnos role in the value chain (§5.7)
7. Customer segmentation (§5.11) — **new in v5**
8. What Ichnos does on this page (§5.8)
9. Roadmap signal (§5.12) — **new in v5**
10. Contact CTA (§5.9)

The render order interleaves new v5 sections with v4 sections. The
new sections take the spec numbering slot above 5.10 (SEO meta, now
renumbered to §5.13) so existing cross-references inside v4 keep
their meaning where the underlying content didn't move.
...
### 5.10 Build stack credibility — `passportContent.js`

**Audience: Catena-X qualification reviewer + technical evaluator (per
§0c).** This sub-section names the technical stack Ichnos works with
as ambient terminology, not as a sales pitch. The Catena-X reviewer
landing on `/passport` reads this paragraph and concludes "this
practice is fluent in the Catena-X / Tractus-X tooling." A first-time
ASEAN supplier visitor skims it; that audience is served by §3.4 and
§5.3-5.5 above.
...
Visual: heading, one explanatory paragraph, then a two-column list of
standards (label in monospace or eyebrow weight, note in body text).
The list is dense on purpose — it reads as a fluency signature, not a
sales-page bullet stack. The pointer to `catena-x.net/en/about-us` from
§5.6 is the only outbound link; this section does not link to each
standard.

**What this section deliberately does NOT include:**

- No mention of the **PCF SaaS product**, the **provisioning
  subscription**, the **pricing model**, or the **PCF wedge**. Those
  belong to the strategy memo and a future product page, not the
  current credibility surface.
- No mention of **ESP / BAP certification** as a current credential —
  see §5.12 below for the measured ambition wording.
- No claim that Ichnos has **deployed** any of these in production at
  scale yet. The framing is "the stack Ichnos works with," not "the
  stack Ichnos has shipped."

### 5.11 Customer segmentation — `passportContent.js`

**Audience: First-time visitor + EU importer + Catena-X reviewer (per
§0c).** This sub-section names the three customer groups Ichnos works
with, so a visitor self-identifies into one of them. It does NOT
disclose the pricing model or which group pays which subscription
tier (those belong to the strategy memo and a future product page).
...
Visual: heading, intro line, then three cards laid out horizontally
on desktop (single column on mobile). Each card has a one-line label
and a 2-3 sentence body. No CTAs on the cards — interest goes through
the page's main Contact CTA (§5.9).

**What this section deliberately does NOT include:**

- No statement that ASEAN suppliers are the **primary payer** of a
  subscription. The memo says they are; the website does not.
- No statement of **pricing structure** (per supplier site, per
  active model, setup fee). Those stay in the strategy memo.
- No marketing language about **why** each group should engage — the
  body text is descriptive ("who they are, what Ichnos does with
  them"), not aspirational.

### 5.12 Roadmap signal — `passportContent.js`

**Audience: Catena-X reviewer + technical evaluator (per §0c).** A
deliberately thin single-paragraph signal of measured ambition. The
strategy memo lays out a full Phase 0 → Phase 3 roadmap; the website
shows only the open, credibility-bearing slice that supports the
consultant-qualification listing.
...
Visual: heading and a single paragraph. No bullets, no timeline graphic.
The intent is to signal direction, not to commit to dates.

**What this section deliberately does NOT include:**

- No mention of the **PCF MVP**, **design partners**, **Industry Core
  Hub integration milestones**, or the **16-week build plan** from the
  strategy memo Phase 0. Those are stealth.
- No mention of **dates** beyond "is working toward" — undisclosed
  timelines protect optionality.
- No claim that ESP / BAP is **applied for** or **granted**. The verb
  is "working toward."

### 5.13 SEO meta — `client/src/constants/seoMeta.js`
...
### 5.14 Structured data — `client/src/constants/structuredData.js`
...
### 9.6 Naming convention — `CatenaX` in code, `Catena-X` in copy

Francesco's explicit instruction in v5: **standardise on `CatenaX` for
every code identifier**. The v4 branch had inconsistent forms
(`CatenaThemeLayout.jsx` without the trailing X, `CatenaXQualifierSpan`
with it). v5 unifies on the explicit-X form everywhere code is read by
the compiler.
...
| --- | --- | --- |
| File names | `CatenaX...` (PascalCase) | `CatenaXThemeLayout.jsx`, `CatenaXQualifierSpan.jsx` |
| React component names | `CatenaX...` | `<CatenaXThemeLayout>`, `<CatenaXQualifierSpan>` |
| JS constants / functions | `CATENA_X_...` or `getCatenaX...` | `CATENA_X_QUALIFICATION_GRANTED`, `getCatenaXFullTitle()` |
| CSS classes | `.theme-catenax` (lowercase, no separator inside `catenax`) | `.theme-catenax`, `.catenax-qualifier-pending` |
| File / module names (constants) | `catenaX...` (camelCase) | `catenaXStatus.js` |
| User-facing copy (HTML, alt text, titles, descriptions) | `Catena-X` (official spelling, hyphenated) | "Official Catena-X Qualified Advisory Provider" |
| Comments inside code | Either `Catena-X` or `CatenaX` is acceptable; prefer the hyphenated form when explaining behaviour, the no-hyphen form when naming an identifier |

**Why distinct forms.** `Catena-X` is the registered Catena-X
Association mark used in every public communication; the hyphen is
visually distinctive and must appear unchanged in user-facing copy.
Code identifiers cannot carry a hyphen (it parses as a minus), so the
de-facto convention in the Tractus-X and Catena-X SDKs is `CatenaX`
(PascalCase) or `catenax` (lowercase, in CSS / file paths). v5 picks
those forms and bans the mixed-case half-states (`Catena_X`,
`CATENA-X`, `Catena-x`, `catena_x`).

**Implementer's contract.** Lint or grep should fail the build if a
code identifier outside the rule appears. Specifically check for:

- `Catena[^X-]` in `.jsx` / `.js` files inside `client/src/` (any
  Catena followed by something other than `X` or `-` flags a likely
  typo like `CatenaThemeLayout` or `Catena_Status`).
- `[Cc]atena[_-][Xx]` in identifier positions (declarations, imports,
  class names) — the hyphen/underscore between Catena and X is
  banned in code.

The v4 branch already contains one violation (`CatenaThemeLayout.jsx`).
Rename to `CatenaXThemeLayout.jsx` in Phase C-D, updating all import
statements in the same commit so no intermediate state of `main`
breaks.
