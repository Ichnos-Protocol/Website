...
### 12.3 Part-2 pre-answers (2026-08-12 · the four technical-approach questions; approach drafting starts after this)

**Q1 status-quo heading/intro → A.** Both survive, verbatim, housed in the slimmed `PASSPORT_STATUS = { heading, intro }` — the `milestones` field (with its two wrong dates) is deleted per §12-Q4; nothing else in the constant changes. The section shell renders **heading → intro → `<RegulatoryTimeline/>` → §7.1 footnote**. §7.1's "replace the block" targets the milestones body, never the section shell — the same reading §12.1 gave the role section. **D rejected:** `regulatoryDates.js` is pure data (`{id, label, date|datePending, deferred, source}` + `VERIFIED_AS_OF`, §1-Mechanism); display copy stays in `passportContent.js` — mixing copy into the date file breaks the single-source discipline it copies from `catenaXStatus.js`. **B/C rejected:** the intro is the page's one citation of Reg. (EU) 2023/1542 ahead of the timeline and is factually clean — v4 deletes wrong dates, not correct context. Contingency (§1 supremacy): if the intro turns out to contain any date string at implementation, it must match §1 or the sentence carrying it is dropped; expected outcome is verbatim survival. The heading "Status quo and milestones" still describes a timeline with a today marker; retitling is out of scope.

**Q2 data-stack bold lead-ins → A, with this exact segmentation.** `PASSPORT_CATENAX` keeps `heading`; `body` is replaced by four sibling fields, mapped by `PassportCatenaXStack.jsx` (no `dangerouslySetInnerHTML` anywhere):

```js
intro:          // §7.3 ¶1 verbatim: "Catena-X is the automotive industry's shared data network, … across dataspaces."
principlesLead: "Its principles matter more than its acronyms.",   // its own short <p>
principles: [   // each rendered <p><strong>{lead}</strong> {text}</p> — colon INSIDE the <strong>
  { lead: "Your data stays yours:",            text: "every company keeps data sovereignty … under which contract policy." },
  { lead: "Trade secrets don't travel:",       text: "data is exchanged bilaterally … never propagates along the chain." },
  { lead: "Every request is verified:",        text: "each participant holds verifiable credentials … at every data request." },
  { lead: "Nothing moves without a contract:", text: "exchanges run through a connector … no central database in between." },
],
closing:        // §7.3 ¶3 verbatim: "Ichnos can run this for you: … without building one."
```

Amendment recorded to §7.3 (copy-author's intent): the quote-block's markdown paragraphing is **source formatting**; the normative content is the character sequence plus the bold spans. Rendering each principle as its own short `<p>` is the sanctioned presentation — it strengthens the scan anchors the lead-ins were written for and serves the §8 mobile mandate (no 120-word wall at 390 px). All strings character-verbatim from §7.3. **B rejected** (re-creates the wall; buys only source-cosmetic fidelity). **C rejected** (deletes spec'd emphasis — the bold is part of the §7.3 content mandate). **D rejected** (paragraph strings without the lead/text split either lose the bold or force HTML-in-strings, which is banned). Corpus note: the new nested strings remain inside the tier-2 vocabulary scan — if the corpus walker only reads flat top-level string exports, extend it to traverse arrays/objects **in the same P6 commit** (guard lands with the shape, §0.3).

**Q3 unstyled classes → C, with A's discipline for the dead eight.** `index.css` gains ONE global `.section-eyebrow` block (small size, letter-spacing, `--color-text-secondary`, spacing) plus `.section-eyebrow--hero` (§3's quieter-on-photo variant; AA against the hero overlay). It lands in **P3** — the earliest consumer; §3's repositioned hero eyebrow is why the modifier exists — and P4/P6 consume the class without redefining it. The eight `.passport-timeline*` / `.stack-band*` classes get **no CSS ever**: their JSX dies in the P6 rebuild and their absence is the fix. Precedent: pivot-3 §3.4 (`.footer-label-img`) — a class shipped in JSX with no rule is a latent defect, repaired where the run already touches the surface. **D rejected** (leaves §3's hero eyebrow unstyled inside this run). **B-without-modifier rejected** (photo-overlay contrast needs differ from in-page bands; one undifferentiated rule forces inline overrides later). Two standing rules from this finding: (i) the new timeline uses a fresh BEM block (`.regulatory-timeline__…`) — the dead `.passport-timeline*` names are never reused; (ii) any new class a phase introduces ships WITH its CSS rule in the same commit — no new latent classes. `index.css` contention: each phase touches only its own scoped block of the file; the shared `.section-eyebrow` block lands once in P3; a file recurring across phases does not by itself breach the cap (the cap counts files per commit).

**Q4 timeline assertions → C, split by what is protected.**
- **`regulatoryDates.test.js`** (new, beside the constant, lands in **P1** — guards first, §0.3): `VERIFIED_AS_OF` present and shaped `\d{4}-\d{2}-\d{2}`; ids unique; `label` and `source` non-empty on every entry; exactly ONE of `date`|`datePending` per entry; dated entries chronologically non-decreasing **in authored order**. Normative mechanism resolved here: the data file is authored in display order and **the component renders authored order, it never sorts** (no date math over `datePending` items); order is a data invariant, enforced in the data test.
- **`RegulatoryTimeline.test.jsx`** (new, beside the organism, lands in **P6**): loops over the imported array — one node per entry (testid `timeline-item-${id}`), the `--deferred` class exactly on `deferred: true` entries, today-marker testid present, footnote interpolates the imported `VERIFIED_AS_OF` (§7.0: zero restated literals). The today marker derives from the runtime clock (`new Date()`, "MMM yyyy" — §7.1's "Aug 2026" was illustrative); tests assert its testid only, never its text.
- **`PassportPage.test.jsx`** keeps ONLY composition: the §12.1-C named section list gains the timeline section's testid; no item-level duplication. **B rejected** (a page test asserting item internals double-fails on every data edit — the §7.0 anti-pattern). **A-alone rejected** (leaves the data invariants unguarded, and the entire §1 table funnels through `regulatoryDates.js` — a component test cannot see an authoring error that renders happily, e.g. wrong order).

**File-set and §9/§10 deltas (recorded here; the table is not rewritten):**
- **P1** = `regulatoryDates.js` + `regulatoryDates.test.js` + the FORBIDDEN widening — §10's "3" now has its named third file; within cap.
- **P1 FORBIDDEN conditional (§0.3 override of §9's "add in P1"):** each of the four claim patterns lands in P1 ONLY if the current corpus is already clean of it; any pattern with live occurrences lands in the commit that deletes the last occurrence (expected: the zero-knowledge/ZKP family, and possibly the hosting phrase, land with P6's §7.3 body replacement). A red P1 is never acceptable.
- **§9 delta — two new FORBIDDEN literals `18 February 2025` and `18 August 2025`,** landing in **P6** with the milestone deletion (earliest green position — adding them in P1 would be red against the still-live milestones). They make §12-Q4's "no trace in shipped source" machine-checkable; if future copy ever legitimately needs the historical date, the pattern is removed with a recorded reason per pivot-3 practice.
- **P3** gains `index.css` (the `.section-eyebrow` block) → 5 files; atomic pre-approval extended: §3's treatment IS the styling — shipping the DOM flip unstyled would leave a broken eyebrow in the hero between commits.
- **P6** gains `RegulatoryTimeline.test.jsx`, plus the organism edits already implied by §12-Q2/Q4 and §12.1 (`PassportCatenaXStack.jsx`, `PassportRoleBand.jsx`, `PassportOffer.jsx`, the `ColumnList` orphan check). **Part-2 authority delegated:** P6 MAY split into sub-commits (e.g. P6a timeline swap · P6b data-stack body · P6c role band + offer strip + de-dupe) — each a semantic flip per §0.2, green before commit, reason recorded; the same delegation covers P4/P5 sequencing under the cap.

### 12.4 Approach pre-answers (2026-08-12 · the five refactoring-approach questions; `refactoring-approach.md` drafting starts after this)

**Resolved conditionals (Traycer code verification 2026-08-12, accepted as normative):**
- The tier-2 corpus scan regexes **raw file text** (`readFileSync` in `vocabulary.test.js`), so the §12.3-Q2 nested shape is scanned as-is. The walker-extension conditional is CLOSED — no P6 walker work.
- All four §9 claim patterns verified clean in the current corpus → **all four land in P1 unconditionally.** §12.3's expectation that the ZKP family trails to P6 is superseded (the live body attributes trust to Verified Credentials; the ZKP claim never shipped). The two dead-date literals (`18 February 2025`, `18 August 2025`) still land in **P6** with the milestone deletion — unchanged.
- Finding 3 (`services-group-*` family entirely unstyled): **accepted latent state, no action this run.** Rule (ii) governs classes a phase *introduces*; existing classes rendering acceptably on inherited defaults are not this run's defect. Exception: if P5's subtitle work makes one of them load-bearing for the new layout, style that one under rule (ii) in P5.

**Q1 Kits gloss → B, with the §7.0 repair.** The gloss is unique content — the page's one plain-English definition of a KIT — not the duplication §7.5 targets; it survives, relocated into `PASSPORT_BUILD_STACK.body` (which already discusses Tractus-X). Splice rules: ONE added or extended sentence; the phrase `Kits (the Tractus-X Development Kits)` stays character-identical; no em-dash; no new factual claims (definitional splice only — do NOT upgrade to the official "Keep It Together" expansion in this run, that would be a new claim to verify). The `:124` assertion is a restated literal — the same §7.0 anti-pattern §12 fixed at `CredentialStrip.test.jsx:77` — so it is FIXED, not retyped: re-point it to assert the build-stack section renders the imported `PASSPORT_BUILD_STACK.body`; if an equivalent constant-driven assertion already covers that section, DELETE `:124` as redundant. **A rejected** (content loss: deleting the term's only definition orphans "KIT" for the semi-technical reader this page targets). **C rejected** (re-opens §12-Q2's strip-down; the offer stays eyebrow + pointer + CTA, no content paragraphs).

**Q2 CSS placement → C (B's principle, correctly sequenced). No styles-first phase.** Rule-before-markup remains permitted (the T2 `--member` precedent stands; rule (ii) is one-directional) but only where the markup is already fully specified. Pre-authoring `.regulatory-timeline__*` for a component not yet designed inverts the design order and all but guarantees a second `index.css` touch in P6 — recreating the churn A exists to avoid, plus dead-CSS risk if the run re-sequences (the §12-Q5 relief valve). The contention A solves is not real: one implementer, sequential phases, per-phase scoped blocks (§12.3-Q3). Placement map: `.section-eyebrow` + `--hero` in **P3** (markup already live on four sites — effect immediate, no latency in either direction); `--advisor`/`--member` sizing in **P4a** with its emitting markup; `.pillar-subtitle` in **P5**; `.regulatory-timeline__*` with the **P6 component build** (same commit as markup + test, per Q4); `.passport-diagram` with the commit that adds each `<img>`.

**Q3 P4 decomposition → A.** P4a sizing (`CredentialLabel.jsx`, `FooterRecognitions.jsx`, `index.css` — independent, purely presentational on existing data). P4b pre-wires `structuredData.js` to `CATENA_X_MEMBERSHIP_NOTE` (1 file — a self-contained single-source improvement; item 8 requires ≥1 consumer, so two consumers in the interim is green, and **hotspot H1 collapses from a same-commit constraint to a sequencing note**). P4c the flip (`catenaXStatus.js`, `credentials.js`, `vocabulary.js`, `CredentialStrip.test.jsx`, `catenaXStatus.test.js` — pre-approved atomic: shape + consumers + guards per §0.2; the `STATUS_STRING_EXPORTS` delta and both test re-points are consumers of the flip and inseparable from it). Each sub-phase carries any test its own change reds (§0.10 makes this automatic). **B rejected on principle:** it buys cap compliance by deferring a standing guard (pivot-3's `STATUS_STRING_EXPORTS`-same-commit rule) — caps yield to atomicity, never the reverse; a cap-met, guard-deferred commit is the anti-pattern itself. **D rejected** (pre-approval is a ceiling, not a target; separable units bisect better). **C dominated by A** (the pre-wire costs one trivial commit and shrinks the irreducible atomic core).

**Q4 P6 shape → A, extended; C's diagram half rejected.** **P6a** builds `RegulatoryTimeline.jsx` + colocated test + its `index.css` block (3 files, green by construction while unmounted — the test renders it). **P6b** swaps: `passportContent.js` (slim `PASSPORT_STATUS` per §12.3-Q1), `PassportPage.jsx` mount, `PassportPage.test.jsx` section list (§12.1-C), delete `PassportStatusTimeline.jsx` (plus its colocated test if one exists), `vocabulary.js` dead-date literals — pre-approved atomic. **P6c** carries the §7.3/§7.4/§7.5 content flips (data-stack shape + `PassportCatenaXStack.jsx`, role band, offer strip + the Q1 relocation, `ColumnList` orphan check, diagram `<img>`s + `.passport-diagram` CSS) and MAY itself split further under the §12.3 delegation. **C's diagram pre-landing rejected:** §0.2 defines the commit boundary as the semantic flip, and the §7.2/§7.4 flip is *prose replaced by diagram* — an add-img-keep-prose commit half-executes it, shipping a duplicate-content intermediate into history with zero verification value (the images are §0.8 static assets; there is no logic to pre-prove — unlike the timeline, where pre-landing de-risks the run's biggest new-code unit). **B rejected** as default (mixing brand-new component code with page rewiring loses A's bisection value) but stands as the **fallback** if a dead-code/unused-export lint rejects the unmounted P6a component (test imports normally satisfy such lints) — merge P6a+P6b rather than weakening any lint.

**Q5 `.section-eyebrow` reach → A.** The base rule carries the full eyebrow treatment, **mixed-case**: small size (≈0.8rem), letter-spacing 0.04–0.08em, `--color-text-secondary`, its spacing — **no `text-transform`**. `--hero` adjusts only what the photo context needs (AA-safe ink on the overlay per §3). Brand ground, recorded as **rule (iii)** beside §12.3-Q3's two: strings this class touches carry proper marks — "Catena-X" in the hero credential line, "Tractus-X" in the build-stack labels — whose casing MUST NOT be visually rewritten by CSS; `text-transform: uppercase` is banned on any class that can contain the marks (same hygiene family as pivot-3 §2.1). The nine `PassportBuildStack` labels receiving the treatment is **accepted and desirable**: currently-unstyled latent markup becomes consistent small-label tokens. If later work wants them distinct, re-class them THEN under rule (ii) — not pre-emptively. **B rejected** (CSS-rewrites third-party name casing, plus a nine-label re-class churn). **C rejected** (empties the base class into a stub and pushes the real treatment into a growing modifier set — the opposite of one-treatment-per-concept). Owner-gate addition: the §8 manual matrix gains one glance at the build-stack labels and both passport-page eyebrows after P3 lands, since the global rule reaches pages P3 otherwise doesn't touch.

---

## 13. Post-run fix round (2026-08-12 · owner review findings; direct commits on main `71b7749` / `927bb5b` / `3e7531d`, full suite 687/687 green)

Francesco's staging review surfaced four issues; fixed directly (no Traycer run — visual-judgment scope). Normative deltas:

1. **§2 amendment — navbar mark:** `Logo` in the navbar is ALWAYS the dual-tone mark (`theme="light"`); the old `/passport → white mark` switch hid the tree on the white navbar. The white mark remains a dark-surface asset (footer). Guarded by the updated `Navbar.test.jsx` passport assertion.
2. **§5.2 SUPERSEDED — font-parity sizing model.** The bounding-box model shipped exactly as specified and was still optically wrong: the advisor 16:9 artwork is mostly illustration, so its "Catena-X" wordmark rendered at ~2/3 of the member's and "Qualified Advisor" at ~4.5px. Measured wordmark fractions: member 0.478 of its canvas height; advisor 0.144 of the 16:9 canvas → 0.318 of the cropped artwork. **New normative rule: the two labels render with equal "Catena-X" wordmark height ⇒ advisor (cropped) height = 1.5× member height.** Values: strip 60/40px, footer 54/36px, §7.4-tunable ±15%, ratio normative.
   - **Display assets are `_cropped.svg` derivatives** (viewBox-trimmed renders of the official files; artwork byte-identical, empty canvas removed). The official `_16x9.svg` masters stay in `/brand`. Clear space is re-supplied as content-box CSS padding — the same treatment the tight-cropped member file always had. Tier-3 item 15 literals now assert the `_cropped` filenames.
3. **Footer credentials restructure:** every credential leads with a visible text title; the two official labels render beneath their titles; marketing notes are footer-omitted (strip keeps them). New optional `footerLabel` field on `CREDENTIALS` — member shows `'Catena-X Association ordinary member'` in the footer only; `label` stays the card text and every image's alt. The brand column's plain-text credential line is DELETED (duplicated the block); guarded by a not-contains assertion.
4. **Positioning line replaced:** `'Battery and EU battery passport advisory for ASEAN.'`, single-sourced in `COMPANY_INFO.tagline` and consumed by the footer, the schema.org organization description opening, the default og:image alt, and (as literals) the static `index.html` head + `index.test.js`. The static head's "Catena-X onboarding" also became "Catena-X integration" (§1 onboarding-wording hygiene).

**Owner follow-ups (open):** regenerate `og-image.jpg` — the graphic still carries the old tagline text baked in; optional cleanups: `LANDING_META.description` still says "EU battery-passport integration" inside its services list (accurate as a service description, left intentionally); `knowsAbout: "ASEAN Catena-X Onboarding"` in structuredData (topic string, acceptable). Deploy path for this round: push `main` (no PR — suite verified green out-of-band in the cloud container, 687/687), then "Sync main → staging".
